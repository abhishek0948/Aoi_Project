  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      <button
        ...
        className="md:hidden fixed top-4 left-4 z-1001 bg-orange-400 text-white p-2 rounded-lg shadow-lg hover:bg-orange-500 transition-colors"
      ...
      {/* Left navigation bar - hidden on mobile, shown on md+ */}
      <div className="hidden md:flex absolute left-0 top-0 h-full w-12 lg:w-16 bg-black/50 flex-col items-center py-4 z-1000 shadow-lg">
        <div className="flex flex-col gap-2 lg:gap-4 mb-auto">
          <button className="w-10 h-10 lg:w-12 lg:h-12 flex items-center justify-center transition-colors relative">
            <svg className="w-6 h-6 lg:w-8 lg:h-8 relative z-10" viewBox="0 0 24 24">
              <defs>
          ...
          <button className="w-10 h-10 lg:w-12 lg:h-12 flex items-center justify-center hover:bg-white/10 transition-colors">
          ...
          <button className="w-10 h-10 lg:w-12 lg:h-12 flex items-center justify-center hover:bg-white/10 transition-colors">
            <svg className="w-5 h-7 lg:w-7 lg:h-9 text-white" fill="currentColor" viewBox="0 0 20 24">
      ...
      {/* Main sidebar - toggleable on mobile, fixed width on md+ */}
      <div className={`fixed md:absolute left-0 md:left-12 lg:left-16 top-0 h-full w-full md:w-80 lg:w-96 bg-white flex flex-col z-1000 shadow-lg transition-transform duration-300 ${
        isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
      }`}>
        <div className="p-3 md:p-4 mt-14 md:mt-0">
          <div className="flex items-center gap-2 md:gap-4 mb-3 md:mb-4">
            <button className="text-gray-600 flex gap-1 md:gap-2 items-center">
              <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
              <div className="w-[1.2px] h-6 md:h-8 bg-gray-400 rounded"></div>
            </button>

            <div className="flex items-center gap-2">
              <h2 className="text-base md:text-lg text-orange-400">Define Area of Interest</h2>
            </div>
          </div>
        ...
        <div className="px-3 md:px-4 pb-3 md:pb-4">
          <label className="block">
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-3 md:p-4 text-center cursor-pointer hover:border-orange-400 hover:bg-gray-50 transition-colors">
        ...
        <div className="pb-3 md:pb-4 shrink min-h-0">
          <AOIList
            features={features}
